...
public class ProjectDao {
    ...
    public void insert(Project project) {
        jdbcTemplate.update(
                "INSERT INTO public.projects(name) VALUES (?);", project.getName());
    }

    public void update(Project project, Integer Id) {
        jdbcTemplate.update(
                "update public.projects SET name=? WHERE id=?", project.getName(), Id);

    }

    public Project findById(int id) {
        return (Project) jdbcTemplate.queryForObject("select * from public.projects where id = ?", new Object[]{id},
